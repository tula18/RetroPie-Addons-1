...
# TRN: store video-config and emu-config in each system-folder instead, this way we
# can have every emulator running different resolutions for each system.
VIDEO_CONF="$CONFIGDIR/$3/videomodes.cfg"
EMU_CONF="$CONFIGDIR/$3/emuvideo.cfg"
DISPMANX_CONF="$CONFIGDIR/all/dispmanx.cfg"
RETRONETPLAY_CONF="$CONFIGDIR/all/retronetplay.cfg"

# TRN: enable/disable tvservice switching, can be usefull if you switch your retropie
# to a hardware-setup with only 1 video-mode (e.g. LCD display).
TVSERVICE_ENABLE=1
